...
function makeRoundTargets(rounds: number, mode: TimeMode): number[] {
  return Array.from({ length: rounds }, () =>
    mode === "random" ? Math.floor(Math.random() * 9) + 3 : mode,
  );
}
...
function FiveSecondsGame() {
  ...
  const standings = useMemo(() => {
    return activeNames.map((name, index) => {
      ...
      const totalError = records.reduce((sum, attempt) => sum + attempt.error, 0);
      return { index, name, records, totalError };
    }).sort((a, b) => a.totalError - b.totalError || a.index - b.index);
  }, [activeNames, attempts]);
  ...
  return <section className="five-v2-shell" aria-live="polite">
    ...
    {phase === "turn" && <div className="five-v2-card five-v2-turn">
      ...
      <p className="five-v2-label">次はこの人</p>
      <h2>{activeNames[playerIndex]}さん</h2>
    ...
    {phase === "running" && <button className="five-v2-stop" type="button" onClick={stopAttempt} aria-label="計測を止める">
      <span>ROUND {roundIndex + 1}</span><small>{activeNames[playerIndex]}さん</small><strong>STOP</strong><p>{target}秒だと思ったらタップ！</p>
    </button>}

    {phase === "result" && latest && <div className="five-v2-card five-v2-result">
      <p className="five-v2-label">{activeNames[playerIndex]}さんの記録</p>
      <div className="five-v2-rank">{rankFor(latest.error)}</div>
    ...
    {phase === "round" && <div className="five-v2-card">
      <p className="five-v2-label">ROUND {roundIndex + 1} 終了</p><h2>現在順位</h2>
    ...
    {phase === "final" && <div className="five-v2-card five-v2-final">
      <p className="five-v2-label">FINAL RESULT</p><div className="five-v2-trophy">🏆</div><h2>{standings[0]?.name}</h2><p>優勝！ 合計誤差 <b>{standings[0]?.totalError.toFixed(2)}秒</b></p>
...
export default function FiveSecondsV2() {
  useEffect(() => {
    ...
    const mount = () => {
      ...
      mountedPage?.querySelectorAll<HTMLElement>(":scope > .play-card").forEach((card) => { card.hidden = false; });
