...
const DEFAULT_NAMES = Array.from({ length: 8 }, (_, index) => `プレイヤー${index + 1}`);
const HOME_PATH = "/sakaba-casino/";
...
export default function FiveSecondsRoute() {
  ...
  return <div className="five-v2-page">
    <header className="game-header five-v2-header">
      <button className="back" type="button" onClick={() => window.location.assign(HOME_PATH)} aria-label="ホームへ戻る">‹</button>
      <div><small>JUST FIVE</small><h1>5秒ぴったり</h1></div>
      <div className="five-route-meta"><small>{APP_VERSION}</small><span>⏱️</span></div>
    </header>
    ...
    <section className="five-v2-shell" aria-live="polite">
      ...
      {phase === "turn" && <div className="five-v2-card five-v2-turn">
        ...
        <button className="secondary five-v2-abort" type="button" onClick={resetToSetup}>ゲームを中止</button>
      </div>}
      ...
      {phase === "running" && <div className="five-v2-running">
        ...
        <button className="five-v2-stop" type="button" onClick={stopAttempt} aria-label="計測を止める"><small>{activeNames[playerIndex]}さん</small><strong>STOP</strong><p>{target}秒だと思ったらタップ！</p></button>
        <button className="secondary five-v2-abort" type="button" onClick={resetToSetup}>ゲームを中止</button>
      </div>}
      ...
      {phase === "final" && <div className="five-v2-card five-v2-final"><p className="five-v2-label">FINAL RESULT</p><div className="five-v2-trophy">🏆</div><h2>{standings[0]?.name}</h2><p>優勝！ 合計誤差 <b>{standings[0]?.totalError.toFixed(2)}秒</b></p>
        ...
        <button className="secondary" type="button" onClick={resetToSetup}>設定を変更</button>
      </div>}
    </section>

    <style>{`
      .five-route-meta { display:flex; align-items:center; gap:10px; }
      .five-route-meta small { color:#735bdc; background:#fff; border-radius:999px; padding:5px 10px; box-shadow:0 3px 12px #6d5bd522; font-weight:800; letter-spacing:.03em; }
    `}</style>
  </div>;
}
